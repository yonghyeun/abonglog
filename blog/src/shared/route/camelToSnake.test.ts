...
describe("camelToSnake 함수 테스트", () => {
  test("원시 타입 값은 변경 없이 그대로 반환한다", () => {
    expect(camelToSnake("helloWorld")).toBe("helloWorld"); // 문자열은 값으로 취급, 키가 아님
  ...
  test("단순 객체의 키를 카멜 케이스에서 스네이크 케이스로 변환한다", () => {
    const input = {
      userId: 1,
      firstName: "홍",
      lastName: "길동",
  ...
  test("이미 스네이크 케이스인 키는 그대로 유지된다", () => {
    ...
    const expected = {
      user_id: 1,
      user_name: "홍길동", // 이미 스네이크 케이스이므로 변경 없음
      user_age: 30
    };
